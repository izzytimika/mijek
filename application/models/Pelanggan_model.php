...
class Pelanggan_model extends CI_model
{
     ...
     public function sliderfood()
    {
        $this->db->select('*');
        $this->db->from('promosi');
       // $this->db->join('fitur', 'promosi.fitur_promosi = 10', 'left');
        $this->db->where('is_show', '1');
         $this->db->where('fitur_promosi', '10');
      //  $this->db->where('tanggal_berakhir>', date("Y-m-d"));
        return $this->db->get()->result_array();
    }
...
public function insertwallet($data_withdraw)
    {
        if($data_withdraw['type']=='topup'){
            
        
        $length = 10;    
 
         $kodetrx = substr(str_shuffle('0123456789'),1,$length);
            $curl = curl_init();
           ...
           $nominal = str_replace(',00',' ', $data_withdraw['jumlah']);
            $transaction_data = array(
			'external_id' 			=> $kodetrx, 
		
			'amount'=> $nominal,
			'payer_email' 			 => 'mitragrahatimika@gmail.com',
			'description' 	 => 'Deposit wallet customer'
		);
		
            // $data['payment_methods'] = 'OVO';
            //  $data['fixed_va'] = 'true'; 
             // $data['callback_virtual_account_id'] = '5ea997a94a9bee0ecba2f47a';
              
            /*
            if ( is_array($invoice_options) ) {
                foreach ( $invoice_options as $key => $value ) {
                    $data['callback_virtual_account_id'] = $cekuser->wallet_id;
                     $data['payment_methods'] = ['MANDIRI'];
                     $data['fixed_va'] = true;
                }
            }
            */
 ...
 CURLOPT_POSTFIELDS =>$payload,
  ...
  CURLOPT_USERPWD => 'xnd_production_AeQYz87a8Q1Cg5WjdePFBoyb3wUbpJIZL154zRzM8Mkyby5nDOouA5iHT83opEZ' . ':' . '',
));
          ...
          // $statusdepo =  $responseObject->status;
        
         
          
             $invourl =$responseObject['invoice_url'];
              $exid = $responseObject['external_id'];
          // if($statusdepo=='pending'){   
 $transaksi = [

            'id_user' => $data_withdraw['id_user'],
            'rekening' => $data_withdraw['rekening'],
            'bank' => $data_withdraw['bank'],
            'nama_pemilik' => $data_withdraw['nama_pemilik'],
            'type' => $data_withdraw['type'],
            'jumlah' => $data_withdraw['jumlah'],
            'invoice_url' =>  $invourl,
             'external_id' => $exid,
            'status' => 0,
         ...
         $kodetrx = substr(str_shuffle('0123456789'),1,$length);
           ...
           $nominal = str_replace(',00',' ', $data_withdraw['jumlah']);
			...
			'amount'=> $nominal,
			'bank_code'=> $data_withdraw['bank'],
			'account_holder_name' 			 => $data_withdraw['nama_pemilik'],
			'account_number'=> $data_withdraw['rekening'],
...
//if($statusdepo=='pending'){   
        $verify = $this->db->insert('wallet', $transaksi);
//}
        return $verify;
    }
...
/*
    ...
    */
    ...
    function get_biaya()
    {
        $this->db->select('id_fitur as id_fitur,'
            . 'fitur,'
            . 'biaya,'
            . 'icon,'
            . 'keterangan_biaya,'
        ...
        $biaya = $this->db->get('fitur');
        $tempBiayaAll = array();
        $tempBiaya = array();
        $queryDiskon = $this->diskon_wallet();
    ...
    public function insert_transaksi($data_req)
    {
        ...
        if ($data_req['order_fitur'] == 1) {
            $diskon = $this->diskon_wallet(1);
            $ha = 0;
            $kreditamuont = explode(".", $data_req['kredit_promo']);
            if ($data_req['pakai_wallet'] == 1) {
                ...
                $data_req['biaya_akhir'] = $data_req['harga'];
            }
        } else {
            $diskon = $this->diskon_wallet(2);
            $ha = 0;
            $kreditamuont = explode(".", $data_req['kredit_promo']);
            if ($data_req['pakai_wallet'] == 1) {
                $ha  = $data_req['biaya_akhir'] - $kreditamuont[0];
                if ($ha <= 0) {
                    $ha = 0;
                }
                $data_req['kredit_promo'] = $kreditamuont[0];
...
<!--[if mso]><table width="100%" cellpadding="0" cellspacing="0" border="0"><tr style="line-height:0px"><td style="padding-right: 10px;padding-left: 10px;" align="center"><![endif]--><a href="' . $linkgoogle . '"><img align="center" alt="Image" border="0" class="center fixedwidth" src="https://firebasestorage.googleapis.com/v0/b/needer-db57e.appspot.com/o/en_badge_web_generic.png?alt=media&amp;token=68681ec1-9a11-48ad-9873-b9e104970594" style="text-decoration: none; -ms-interpolation-mode: bicubic; border: 0; height: auto; width: 100%; max-width: 206px; display: block;" title="Image" width="206" data-iml="5150976.220000001"></a><a>
<!--[if mso]></td></tr></table><![endif]-->
</a></div></div><a>
<!--<![endif]-->
</a></div><a>
...
<!--[if (mso)|(IE)]></td></tr></table><![endif]-->
    ...
    {
        $fitur = $data_transaksi['order_fitur'];
        $get_diskon = $this->diskon_by_fitur($fitur)->row();
        $diskon = $get_diskon->nilai;

        if ($data_transaksi['pakai_wallet'] == 1) {
            $total_diskon = ($data_transaksi['harga'] + $total_belanja['total_belanja']) * $diskon / 100;
            $biaya_akhir = $data_transaksi['harga'] + $total_belanja['total_belanja'] - $total_diskon;
        } else {
            $biaya_akhir = $data_transaksi['harga'] + $total_belanja['total_belanja'];
        }
